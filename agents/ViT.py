...
from graphs.models.ViT import ViT
...
class ViTAgent(BaseAgent):
    ...
    def __init__(self, config, config_dict, gpu_ids, mode):
        wandb.init(project="Vision-Transformer", entity="jonggyujang0123", config=config_dict, name=f'{config.agent}-lr-{config.learning_rate}', group=f'{config.agent}_{mode}')
        super().__init__(config)
        # define models
        model_config = config_dict["ViT"]
        self.model = ViT(**model_config)
#        self.model = ERF(self.config, pretrained_enc)
        ...
        # define data_loader
        self.data_loader = CIFAR100DataLoader(config=config)
...
#            self.test()
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader.train_loader):
            ...
            if batch_idx % self.config.log_interval == 0:
                self.logger.info('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    self.current_epoch, batch_idx, len(self.data_loader.train_loader),
    ...
    def test(self):
        ...
        self.logger.info('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            test_loss, correct, len(self.data_loader.test_loader),
            100. * correct / len(self.data_loader.test_loader)))
        wandb.log({"acc": 100. * correct / len(self.data_loader.test_loader)})
    ...
    def validate(self):
        ...
        val_loss /= len(self.data_loader.valid_loader)
        self.logger.info('\nValidation set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            val_loss, correct, len(self.data_loader.test_loader),
            100. * correct / len(self.data_loader.test_loader)))
        return correct
